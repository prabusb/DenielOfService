...
public class Proxy {
    ...
    static final int      BUFSIZE=5120; // size of data transfer buffer
                    ...
                    try {
                        ...
                        while (sel.select() > 0) {
                            ...
                            for (Iterator it=ready_keys.iterator(); it.hasNext();) {
                                ...
                                System.out.println("checking address" + tmp.socket().getLocalSocketAddress().toString());
                                if (key.isReadable()) { // data is available to be read from tmp
                                    if (tmp == in_channel) {
                                    	...
                                    	bfr = new BufferedReader ( new InputStreamReader( tmp.socket().getInputStream()));
                                    	String result = null;
                                    	System.out.println("checking inputsetram:" + bfr.toString());
        ...
        while (true) {
            ...
            if (num < 0)
                ...
                if (num == 0)
                    return true;
            buf.flip();
          System.out.println("checking size buffer:" + num);
            if (verbose) {
                log("relay", printRelayedData(toString(from), toString(to), buf.remaining()));
            }
            if (debug) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            p=new Proxy(InetAddress.getByName("localhost"), 8082, InetAddress.getByName("localhost"), 8084, true, true, mapping_file);
            p.start();
        }
        catch (Throwable ex) {
            ex.printStackTrace();
            ...
            try {
                while(t != null) {
                    ...
                    if (verbose) {
                        ...
                        log("run", printRelayedData(toString(in_sock), toString(out_sock), num));
                    }
                    if (debug) {
                        sb=new StringBuilder();
